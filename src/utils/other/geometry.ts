...
export const verticalLineEquation = (line: Line, point: Point) => {
  ...
  const verticalLine: Equation = { k, b } as any
 ...
 * @returns 两线段交点坐标，平行线或线段不相交返回null
 */
export const disPointOnLineByEquation = (equation: Equation, point: Point, dis: number, inside = true) => {
  const { k } = equation
...
export const isArcReverse = (A: Point, B: Point, C: Point, d = 15) => {
  const { start: startPoint, end: endPoint } = lineToStartEndPoint(angleEquidistantPoints(A, B, C, d))
  ...
  const reverse = (sitaA > sitaC && sitaA - sitaC < Math.PI) || (sitaC > sitaA && sitaC - sitaA > Math.PI)
  return reverse
}
